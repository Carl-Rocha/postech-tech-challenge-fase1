...
const NewTransaction: React.FC<NewTransactionProps> = ({
  ...
  const handleTransactionSubmit = async () => {
    setErro(null);
    if (!vm.valido) {
      setErro("Preencha tipo e um valor vÃ¡lido (ex.: 123,45).");
      return;
    }
    try {
      setEnviando(true);
      ...
      else console.log("TransaÃ§Ã£o a ser concluÃ­da:", submitData);
  ...
  const modalTitle = editingTransaction ? "Editar transaÃ§Ã£o" : "Nova transaÃ§Ã£o";
  const buttonText = editingTransaction ? "Salvar alteraÃ§Ãµes" : "Concluir transaÃ§Ã£o";
